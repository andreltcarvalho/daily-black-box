...
export default function App() {
  ...
  return <div className="app-shell"><TitleBar/>
    <aside className="sidebar"><a href="#main" className="skip-link">Ir para conteúdo</a><nav aria-label="Navegação principal">{([['day', 'Hoje', 'day'], ['history', 'Histórico', 'history'], ['settings', 'Configurações', 'settings']] as const).map(([id, label, icon]) => <button key={id} className={view === id ? 'nav-button selected' : 'nav-button'} title={label} aria-current={view === id ? 'page' : undefined} onClick={() => go(id)}><Icon name={icon}/>{label}</button>)}</nav><div className="sidebar-bottom"><span className="local-dot"/>Dados neste computador<p>Sem nuvem. Sem avaliação<br/>de produtividade.</p></div></aside>
    <main id="main" className="main-content"><header className="page-header"><div><h1>{view === 'settings' ? 'Configurações' : view === 'history' ? 'Histórico do dia' : 'Hoje'}</h1><p>{view === 'settings' ? 'Controle o que é observado e como aparece para você.' : humanDate}</p></div><div className="header-actions"><span className={`tracking-status ${stateLabel === 'Ativo' ? 'active' : ''}`} role="status"><i/>{stateLabel}</span><button className="primary-button" disabled={!status || busy || !!status.error || !status.settings.vdi} onClick={() => { void act('set_paused', { paused: !status?.settings.paused }); }}><Icon name={status?.settings.paused ? 'play' : 'pause'} size={17}/>{status?.settings.paused ? 'Iniciar / retomar' : 'Pausar'}</button></div></header>
      ...
      </main><footer className="page-footer"><span>{status && !status.settings.paused && !status.error ? `Agora: ${sourceLabel(status.activity.source, status.activity.hostname, status.activity.app_name)}` : 'Você controla quando observar.'}</span><span>{status?.last_saved_utc ? `Última gravação ${new Date(status.last_saved_utc).toLocaleTimeString('pt-BR')}` : 'Nenhuma atividade gravada'}</span></footer>
    {deleteScope && (
